...
When `sparkify` is part of the published package set, `release.yml` also moves the major action tag `v1` to the release commit so consumers can keep using:

```yaml
- uses: SparkAIUR/sparkify@v1
```

You can verify the major tag target with:
